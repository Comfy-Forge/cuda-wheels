...
# ARCH FIX FIRST -- must run on EVERY platform/torch combo, so it lives
# BEFORE the windows/torch gate below (which sys.exit(0)s on most combos
# and would otherwise make this unreachable; that exact mistake shipped
# once).
